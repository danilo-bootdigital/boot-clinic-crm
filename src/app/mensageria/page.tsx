...
export default function WhatsAppPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/mensageria/status').then((r) => (r.ok ? r.json() : null)).then((d) => setEvolution(d?.configured ?? false)).catch(() => setEvolution(false))
  }, [router])

  async function createConversation(e: React.FormEvent) {
    e.preventDefault()
    setError(null)
    const res = await fetch('/api/mensageria/conversations', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) })
