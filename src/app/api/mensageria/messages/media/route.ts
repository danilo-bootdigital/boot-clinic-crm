...
// POST /api/mensageria/messages/media  (multipart/form-data: file, conversationId, caption?)
  ...
  try {
    ...
    if (conv.channel !== Channel.WHATSAPP) {
      return NextResponse.json(
        { error: `Envio de mídia ainda não suportado no canal ${conv.channel}` },
        { status: 501 }
      );
    }
    ...
    let up;
    try {
      up = await uploadMessagingMedia({
        companyId: dbUser!.companyId, conversationId: conv.id, messageId: msg.id,
        fileName: file.name, contentType: declaredMime, bytes,
      });
    } catch (e) {
      await prisma.message.update({ where: { id: msg.id }, data: { status: 'FAILED', mediaStatus: 'FAILED', failedAt: new Date(), errorMessage: 'falha no upload' } });
    ...
    try {
      ...
      await prisma.message.update({ where: { id: msg.id }, data: { status: 'FAILED', mediaStatus: 'FAILED', failedAt: new Date(), errorMessage: 'falha ao registrar anexo' } });
    ...
    await prisma.message.update({ where: { id: msg.id }, data: { mediaStatus: 'AVAILABLE' } });
    ...
    await prisma.conversation.update({
      where: { id: conv.id },
      data: { lastMessage: content, lastMessageAt: new Date(), accountId: conv.accountId ?? usedInstanceId ?? undefined },
    });
