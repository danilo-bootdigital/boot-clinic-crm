...
vi.mock('@/lib/storage/messaging-storage', () => ({ createWhatsappMediaSignedUrl: vi.fn() }));

import { prisma } from '@/lib/db/prisma';
import { GET } from '@/app/api/mensageria/attachments/[id]/route';
import { resolveModuleUser } from '@/lib/api/session';
import { createWhatsappMediaSignedUrl } from '@/lib/storage/messaging-storage';
import type { PrismaMock } from '@/test/prisma-mock';

const db = prisma as unknown as PrismaMock;
const req = () => new NextRequest('http://localhost/api/mensageria/attachments/x', { method: 'GET' });
...
async function seedAttachment(companyId: string, extra: any = {}) {
  return db.messageAttachment.create({ data: { companyId, messageId: 'm1', storagePath: `${companyId}/c/m/uuid-a.jpg`, mimeType: 'image/jpeg', sizeBytes: 100, originalFileName: 'a.jpg', ...extra } });
}
