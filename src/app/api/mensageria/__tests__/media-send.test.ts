...
vi.mock('@/lib/storage/messaging-storage', () => ({ uploadMessagingMedia: vi.fn(), deleteWhatsappMedia: vi.fn() }));
vi.mock('@/lib/messaging/adapters/whatsapp/evolution', () => ({ sendMediaForConversation: vi.fn(), sendAudioForConversation: vi.fn() }));

import { prisma } from '@/lib/db/prisma';
import { POST } from '@/app/api/mensageria/messages/media/route';
...
function mediaReq(opts: { bytes: Uint8Array; name: string; type: string; conversationId: string; caption?: string; companyId?: string }) {
  ...
  return new NextRequest('http://localhost/api/mensageria/messages/media', { method: 'POST', body: fd });
}
...
beforeEach(async () => {
  ...
  vi.mocked(uploadMessagingMedia).mockReset();
  ...
  vi.mocked(uploadMessagingMedia).mockResolvedValue({ path: 'A/convA/m/uuid-a.jpg', mimeType: 'image/jpeg', sizeBytes: 1024, checksum: 'abc', originalFileName: 'a.jpg' } as any);
...
describe('POST /messages/media — sucesso', () => {
  it('imagem JPEG válida → 201, mensagem + anexo, mediaStatus AVAILABLE, SENT', async () => {
    ...
    expect(await db.message.count({ where: { companyId: 'A' } })).toBe(1);
    expect(await db.messageAttachment.count({ where: { companyId: 'A' } })).toBe(1);
  });
  ...
  it('companyId do frontend é IGNORADO (usa a sessão)', async () => {
    ...
    expect(res.status).toBe(201);
    const att = (await db.messageAttachment.findMany({}))[0];
  ...
  it('ÁUDIO (WAV) → messageType AUDIO, enviado como nota de voz (sendAudioForConversation)', async () => {
    vi.mocked(uploadMessagingMedia).mockResolvedValue({ path: 'A/convA/m/uuid.wav', mimeType: 'audio/wav', sizeBytes: 128, checksum: 'z', originalFileName: 'nota.wav' } as any);
...
describe('POST /messages/media — validação', () => {
  it('conversa de outra empresa → 404 (isolamento)', async () => {
    ...
    expect(vi.mocked(uploadMessagingMedia)).not.toHaveBeenCalled();
  });
...
describe('POST /messages/media — consistência transacional', () => {
  it('falha no storage → mensagem FAILED, sem anexo, 502', async () => {
    vi.mocked(uploadMessagingMedia).mockRejectedValue(new Error('storage down'));
    const res = await POST(mediaReq({ bytes: jpeg(), name: 'a.jpg', type: 'image/jpeg', conversationId: convA.id }));
    expect(res.status).toBe(502);
    expect(await db.messageAttachment.count({})).toBe(0);
    const msg = (await db.message.findMany({ where: { companyId: 'A' } }))[0];
    expect(msg.status).toBe('FAILED');
    expect(msg.mediaStatus).toBe('FAILED');
  });

  it('falha no banco após upload → compensa (deleta arquivo), 500, sem órfão', async () => {
    const orig = db.messageAttachment.create.bind(db.messageAttachment);
    vi.spyOn(db.messageAttachment, 'create').mockRejectedValueOnce(new Error('db down'));
    const res = await POST(mediaReq({ bytes: jpeg(), name: 'a.jpg', type: 'image/jpeg', conversationId: convA.id }));
