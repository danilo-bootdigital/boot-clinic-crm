...
vi.mock('@/lib/messaging/adapters/whatsapp/evolution', () => ({ getMediaBase64: vi.fn() }));
vi.mock('@/lib/storage/messaging-storage', () => ({ uploadMessagingMedia: vi.fn() }));
...
function post(body: any) {
  return POST(new NextRequest(`http://localhost/api/mensageria/webhook?token=${TOKEN}`, { method: 'POST', body: JSON.stringify(body), headers: { 'content-type': 'application/json' } }));
}
...
beforeEach(async () => {
  ...
  await db.channelAccount.create({ data: { companyId: COMPANY, channel: 'WHATSAPP', providerConfig: { instanceName: 'clinic_A' }, isPrimary: true, webhookToken: TOKEN, status: 'CONNECTED' } });
});
...
describe('webhook — recebimento de mídia (imagem/documento)', () => {
  it('imagem: baixa, armazena e cria anexo (mediaStatus AVAILABLE)', async () => {
    ...
    vi.mocked(uploadMessagingMedia).mockResolvedValue({ path: `${COMPANY}/c/m/uuid-foto.jpg`, mimeType: 'image/jpeg', sizeBytes: 3, checksum: 'x', originalFileName: 'foto.jpg' } as any);
  ...
  it('imagem sem legenda: placeholder no content, mas mídia baixada', async () => {
    ...
    vi.mocked(uploadMessagingMedia).mockResolvedValue({ path: `${COMPANY}/c/m/uuid-f.jpg`, mimeType: 'image/jpeg', sizeBytes: 3, originalFileName: 'f.jpg' } as any);
  ...
  it('ÁUDIO recebido (audio/ogg; codecs=opus) é baixado e armazenado', async () => {
    ...
    vi.mocked(uploadMessagingMedia).mockResolvedValue({ path: `${COMPANY}/c/m/uuid-midia.ogg`, mimeType: 'audio/ogg', sizeBytes: 3, originalFileName: 'midia.ogg' } as any);
