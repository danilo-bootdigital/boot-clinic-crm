...
export interface WebhookEventLog {
  ...
  /** Canal de origem do evento (diretriz §4.3). */
  channel?: Channel | null;
  /** Conta de canal onde o evento caiu. */
...
export async function purgeOldWebhookEvents(olderThan: Date): Promise<number> {
  const res = await prisma.channelWebhookEvent.deleteMany({ where: { createdAt: { lt: olderThan } } });
  return res.count;
}
...
export async function logWebhookEvent(e: WebhookEventLog): Promise<void> {
  try {
    ...
    console.error('[mensageria-webhook] falha ao gravar evento de observabilidade:', err);
  }
}
